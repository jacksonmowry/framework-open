...
/**
 * The neuro namespace contains all of the classes defined by and used by the
 * TENNLab open source neuromorphic computing framework.   In general, the
 * documentation for each component is in the markdown directory:
 *
...
/**
 * This first group of definitions facilitates using a hash table for nodes and
 * edges.
 */
...
/**
 * The Property class represents a specific configurable property within a node,
 * edge, or network. Please read the documentation in
 * `markdown/framework_properties.md` for a description of both the `Property`
 * and `Property_Pack` classes.
 */

class Property {
  public:
    /** * Each property has an associated data type chosen from this
     * enumeration. */
    enum Type : signed char {
        INTEGER = 'I', /**< Numerical, integer value */
        DOUBLE  = 'D', /**< Numerical, double precision (64 bits) floating point
                          value */
    ...
    Property(const string& dname, int idx, int len, double dmin, double dmax,
             Type dtype);
    ...
    string
    pretty_json() const; /**< Create a json string that's better than dump(). */
...
/**
 * A PropertyPack encompasses a grouping of properties for a node, edge, and
 * network all within the same structure. This ensures you have a coherent set
 * of properties which make sense for your desired processor(s).  Please see
 * `markdown/framework_properties.md` for documentation.
 */
...
class PropertyPack {
  public:
    ...
    /**
     * This allows a PropertyPack to work like a native tuple. A developer can
     * call `std::tie` to quickly dump the node, edge, and network property maps
     * in a single line of code.
     */
    operator tuple<PropertyMap&, PropertyMap&, PropertyMap&>() {
        return tuple<PropertyMap&, PropertyMap&, PropertyMap&>{nodes, edges,
                                                               networks};
    }

    /* Adding properties to a property pack. */

    int add_node_property(const string& name, double dmin, double dmax,
                          Property::Type type, int cnt = 1);
    int add_edge_property(const string& name, double dmin, double dmax,
                          Property::Type type, int cnt = 1);
    int add_network_property(const string& name, double dmin, double dmax,
                             Property::Type type, int cnt = 1);
};
 ...
 * Please read the documentation in `markdown/framework_network.md for
 * infomation about this class.
 */
class Node {
  public:
    ...
    vector<double> coordinates; /**< Optional -- useful if you are writing or
                                   using a visualization */
    string name; /**< Optional -- can be useful for viz's or hand-tooling
                    networks */
    ...
    void set(const string& name,
             double val); /**< Set a value by its name in the PropertyPack */
...
class Edge {
  public:
    ...
    vector<double>
        control_point; /**< Optional. Bezier control point(s) for displaying. */
...
class Network {
  public:
    ...
    void clear(bool include_properties); /**< Clear network, optionally clear
                                            properties. */
    ...
    void
    set_properties(const PropertyPack& properties); /**< Set the PropertyPack */
    PropertyPack get_properties() const;            /**< Get the PropertyPack */

    bool is_node_property(
        const string& name) const; /**< Query the node properties for name. */
    bool is_edge_property(
        const string& name) const; /**< Query the edge properties for name. */
    bool is_network_property(const string& name)
        const; /**< Query the network properties for name. */

    const Property*
    get_node_property(const string& name) const; /**< Get the node property. */
    const Property*
    get_edge_property(const string& name) const; /**< Get the edge property. */
    const Property* get_network_property(
        const string& name) const; /**< Get the network property. */
    ...
    Node*
    add_or_get_node(uint32_t idx); /**< Return node with id , or create it */
    void remove_node(uint32_t idx,
                     bool force = false); /**< Delete node - if force=false,
                                             error on IO nodes. */
    void rename_node(uint32_t old_name,
                     uint32_t new_name); /**< Change node's id */
    ...
    Edge* get_edge(uint32_t fr,
                   uint32_t to) const; /**< Analogous to get_node() */
    Edge* add_or_get_edge(uint32_t fr,
                          uint32_t to); /**< Analogous to add_or_get_node() */
    void remove_edge(uint32_t fr,
                     uint32_t to); /**< Analogous to remove_node() */

    /* Input and output nodes */

    int add_input(uint32_t idx); /**< Add the next input */
    Node*
    get_input(int input_id) const; /**< Return a pointer to the given input */
    int num_inputs() const;        /**< The number of input nodes */

    int add_output(uint32_t idx); /**< Add the next output */
    Node* get_output(
        int output_id) const; /**< Return a pointer to the given output */
    int num_outputs() const;  /**< The number of output nodes */

    /* The Associated Data JSON */

    void set_data(const string& name,
                  const json& data); /**< Add/Set key/val pair to JSON */
    ...
    void randomize_property(MOA& moa,
                            const string& pname); /**< Randomize single value */
    void randomize_property(MOA& moa, Node* n,
                            const string& pname); /**< Randomize single value */
    void randomize_property(MOA& moa, Edge* n,
                            const string& pname); /**< Randomize single value */

    void
    randomize_property(MOA& moa, /**< Randomize a single value in the vector. */
                       const Property& p, vector<double>& values);

    void randomize(
        const json& params); /**< Generate a random network (unimplemented) */

    /* Pruning and Sorting */

    void prune(); /**< Prune nodes and edges not on an I/O path */
    void make_sorted_node_vector();   /**< Sort the nodes by id, nothing if
                                         already sorted. */
    ...
    NodeMap::iterator
    begin(); /**< Beginning of the nodes in the node hash table. */
    NodeMap::iterator end(); /**< End of the nodes in the node hash table. */

    EdgeMap::iterator
    edges_begin(); /**< Beginning of the edges in the edge hash table. */
    EdgeMap::iterator
    edges_end(); /**< End of the edges in the edge hash table. */
...
/**
 * A Spike is a simple data structure to represent a tuple of id, time, and
 * value.
 */
struct Spike {
    ...
    double value; /**< On platforms which support variable values, this is the
 ...
 * simulator for a processor (or implement the systems code to connect a
 * physical processor to the framework), then these are the methods that you
 * need to implement.  Please read the documentation in
 * markdown/framework_processor (relative to the main framework directory) for
 * information about these methods and implementing a processor to match this
 * interface.
 */
class Processor {
  public:
    ...
    /* Queue spike(s) as input to a network or to multiple networks.
       ...
       -1 and 1, and then the processor can convert its value to the proper
       range.

       When normalized == false, the value is simply passed to the
       neuroprocessor. */

    virtual void apply_spike(const Spike& s, bool normalized = true,
                             int network_id = 0)     = 0;
    virtual void apply_spike(const Spike& s, const vector<int>& network_ids,
                             bool normalized = true) = 0;

    virtual void apply_spikes(const vector<Spike>& s, bool normalized = true,
                              int network_id = 0)     = 0;
    ...
    virtual bool track_output_events(int output_id, bool track = true,
                                     int network_id = 0) = 0;
    virtual bool track_neuron_events(uint32_t node_id, bool track = true,
                                     int network_id = 0) = 0;
    ...
    virtual vector<double> neuron_pre_charges(int network_id = 0) = 0;

    /* Synapse data from all synapses. */

    virtual void synapse_weights(vector<uint32_t>& pres,
                                 vector<uint32_t>& posts, vector<double>& vals,
                                 int network_id = 0) = 0;

    /* Remove state, keep network loaded */
    virtual void clear_activity(int network_id = 0) = 0;

    /* Network and Processor Properties.  The network properties correspond to
       the Data field in the network, nodes and edges.  The processor properties
       are so that applications may query the processor for various properties
       (e.g. input scaling, fire-on-threshold vs fire-over-threshold). */
    ...
    /* get_params() returns the json that you can use to recreate the processor.
       It doesn't have to be the same json as was used to create the processor;
       just json to create the same processor. */
...
/* This calls track_output_events for all of the output neurons in the given
   network. If it fails, it will undo the track_output_events() calls that it
   made previously. */
...
/* If node n is the ith smallest node by node id, and the jth smallest node
   ...
   neuron_last_fires(), neuron_vectors() and neuron_charges(), and also with the
   fact that nodes are stored in a hash table.  */
...
/* This is a heavyweight procedure.  It assumes that *n* has been loaded on *p*.
   It will call run(1) duration times, and then return a json with two keys.
   Each val is a vector of vectors.  The outer vector has an entry for each
   timestep.

    "spike_raster": The inner vector is 0 or 1 for each neuron. The values are
   presented in the same order as sorted_node_vector. "charges": The inner
   vector contains the charge of each neuron (doubles).  The values are
   presented in the same order as sorted_node_vector.
 */

json run_and_track(int duration, Processor* p, int network_id = 0);

/* This copies n, but sets the synapse weights from the network that is on the
 * processor. */
