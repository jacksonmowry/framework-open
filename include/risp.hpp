...
class Network {
  public:
    ...
    Network(neuro::Network* net, double _spike_value_factor,
            double _min_potential, char leak, bool _run_time_inclusive,
            bool _threshold_inclusive, bool _fire_like_ravens, bool _discrete,
    ...
    vector<double> neuron_pre_charges();
  ...
  protected:
    ...
    Synapse* add_synpase(uint32_t from_id, uint32_t to_id, double weight,
                         uint32_t delay);
    ...
    void
    clear_tracking_info(); /**< Clear out all tracking info to begin run() */
     ...
     *  Each subvector stores a set of events, which is composed of neuron and
     * charge change.
     */
    vector<vector<std::pair<Neuron*, double>>> events;

    long long
        neuron_fire_counter; /**< This is what total_neuron_counts() returns. */
    long long neuron_accum_counter; /**< This is what total_neuron_accumulates()
                                       returns. */
    int overall_run_time;           /**< This is what get_time() returns. */
    bool run_time_inclusive;  /**< Do we run for duration+1 (true) or duration
                                 (false) */
    bool threshold_inclusive; /**< Do we spike on >= or >. */
    double min_potential;  /**< At the end of a timestep, pin the charge to this
                              if less than. */
    bool fire_like_ravens; /**< Register neuron fires one step later, so RISP
                              can match RAVENS. */
    ...
    vector<Neuron*> to_fire; /* To make RISP like RAVENS, this lets you fire a
                                timestep later. */

    vector<double> _neuron_pre_charges;
};
...
class Processor : public neuro::Processor {
  public:
    ...
    vector<double> neuron_pre_charges(int network_id = 0);
