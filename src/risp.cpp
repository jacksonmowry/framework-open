...
static json risp_spec = {{"min_weight", "D"},
                         {"max_weight", "D`"},
                         {"max_delay", "I"},
                         {"min_threshold", "D"},
                         {"max_threshold", "D"},
                         ...
                         {"stds", "A"},
                         {"Necessary",
                          {"max_delay", "min_threshold", "max_threshold",
                           "min_potential", "discrete"}}};

/** initialization for Neuron, Synapse  */
Neuron::Neuron(uint32_t node_id, double t, bool l)
    : charge(0), threshold(t), last_check(-1), last_fire(-1), fire_counts(0),
      leak(l), id(node_id), check(false) {};

Synapse::Synapse(double w, uint32_t d, Neuron* to_n)
    : weight(w), to(to_n), delay(d) {};
...
Network::Network(neuro::Network* net, double _spike_value_factor,
                 double _min_potential, char leak, bool _run_time_inclusive,
                 bool _threshold_inclusive, bool _fire_like_ravens,
                 bool _discrete, bool _inputs_from_weights,
                 uint32_t _noisy_seed, double _noisy_stddev,
                 vector<double>& _weights, vector<double>& _stds) {
    ...
    for (eit = net->edges_begin(); eit != net->edges_end(); ++eit) {
        edge = eit->second.get();
        add_synpase(edge->from->id, edge->to->id, edge->get("Weight"),
                    edge->get("Delay"));
    }
}
...
Neuron* Network::get_neuron(uint32_t node_id) {
    ...
    if (it == neuron_map.end()) {
        snprintf(buf, 200,
                 "risp::Network::get_neuron() - %u is not in the neuron map\n",
                 node_id);
...
bool Network::is_valid_input_id(int input_id) {
    return !(input_id < 0 || input_id >= (int)inputs.size() ||
             inputs[input_id] == -1);
}

bool Network::is_valid_output_id(int output_id) {
    return !(output_id < 0 || output_id >= (int)outputs.size() ||
             outputs[output_id] == -1);
}
...
Neuron* Network::add_neuron(uint32_t node_id, double threshold, bool leak) {
    ...
    if (is_neuron(node_id)) {
        snprintf(
            buf, 200,
            "risp::Neuron::add_neuron() - %u is already in the neuron map\n",
            node_id);
        throw SRE((string)buf);
    }
    ...
    if (!threshold_inclusive) {
        n->threshold =
            (discrete) ? (n->threshold + 1) : (n->threshold + 0.0000001);
    }
...
Synapse* Network::add_synpase(uint32_t from_id, uint32_t to_id, double weight,
                              uint32_t delay) {
    Neuron *from, *to;
    Synapse* syn;
    unordered_map<uint32_t, Neuron*>::const_iterator it;
    ...
    if (!is_neuron(from_id)) {
        snprintf(buf, 200,
                 "risp::Network::add_synpase() - node %u does not exist",
                 from_id);
        throw SRE((string)buf);
    }
    if (!is_neuron(to_id)) {
...
void Network::add_input(uint32_t node_id, int input_id) {
    ...
    if (!is_neuron(node_id)) {
        snprintf(buf, 200,
                 "risp::Network::add_input() - node %u does not exist",
                 node_id);
        throw SRE((string)buf);
    }
    if (input_id < 0) {
        throw SRE("risp::Network::add_input() - input_id < 0");
    }
    if (input_id >= (int)inputs.size()) {
        inputs.resize(input_id + 1, -1);
    }
...
void Network::add_output(uint32_t node_id, int output_id) {
    ...
    if (!is_neuron(node_id)) {
        snprintf(buf, 200,
                 "risp::Network::add_output() - node %u does not exist",
                 node_id);
        throw SRE((string)buf);
    }
    if (output_id < 0) {
        throw SRE("risp::Network::add_output() - output_id < 0");
    }
    if (output_id >= (int)outputs.size()) {
        outputs.resize(output_id + 1, -1);
    }
...
void Network::process_events(uint32_t time) {
    ...
    /*
      ...
      pros:
        ...
        When the timestep is very big, which will be for optimized whetstone's
     conv2d, It may uses several GB memory.
    ...
    */
    ...
    this->_neuron_pre_charges.resize(this->sorted_neuron_vector.size());
    for (size_t i = 0; i < this->sorted_neuron_vector.size(); i++) {
        this->_neuron_pre_charges[i] = this->sorted_neuron_vector[i]->charge;
    }
            ...
            if (n->charge >= n->threshold) {
                for (j = 0; j < n->synapses.size(); j++) {
                    ...
                    if (weights.size() == 0) {
                        ...
                        weight = rng.Random_Normal(weights[int(syn->weight)],
                                                   stds[int(syn->weight)]);
                    }
                    if (noisy_stddev != 0) {
                        weight = rng.Random_Normal(weight, noisy_stddev);
                    }
...
void Network::apply_spike(const Spike& s, bool normalized) {
    ...
    if (!is_valid_input_id(s.id)) {
        ...
        throw SRE((string) "risp::Network::apply_spike() - input_id " + buf +
                  " is not valid");
    }
    ...
    if (inputs_from_weights) {
        if (!normalized) {
            ...
            if (index >= weights.size()) {
                ...
                es = "risp::Network::apply_spike() - value must be between 0 "
                     "and ";
                es += buf;
        ...
        if (normalized) {
            v = (discrete) ? floor(s.value * spike_value_factor)
...
void Network::run(double duration) {
    ...
    if (duration < 0) {
        throw SRE("risp::Network::run() - duration < 0");
    }
    ...
    /* shift extra spiking events to the beginning from the previous run()
     * call*/
    if ((int)events.size() > run_time + 1) {
        for (i = run_time + 1; i < events.size(); i++) {
            events[i - run_time - 1] =
                std::move(events[i]); // Google tells me this is O(1)
        }
    }
...
int Network::output_count(int output_id) {
    ...
    if (!is_valid_output_id(output_id)) {
        snprintf(buf, 200,
                 "risp::Network::output_count() - output id %u is not valid",
                 output_id);
        throw SRE((string)buf);
    }
...
double Network::get_time() { return (double)overall_run_time; }
...
double Network::output_last_fire(int output_id) {
    ...
    if (!is_valid_output_id(output_id)) {
        snprintf(
            buf, 100,
            "risp::Network::output_last_fire() - output_id %u is not valid",
            output_id);
        throw SRE((string)buf);
    }
...
vector<double> Network::output_last_fires() {
    ...
    for (i = 0; i < outputs.size(); i++) {
        if (outputs[i] != -1) {
            rv.push_back(neuron_map[outputs[i]]->last_fire);
        }
    }
    return rv;
}
...
vector<double> Network::output_vector(int output_id) {
    ...
    if (!is_valid_output_id(output_id)) {
        snprintf(
            buf, 100,
            "risp::Network::output_last_vector() - output_id %u is not valid",
            output_id);
        throw SRE((string)buf);
    }
    return neuron_map[outputs[output_id]]
        ->fire_times; // If tracking is turned off, this is empty.
}
...
vector<vector<double>> Network::output_vectors() {
    ...
    for (i = 0; i < outputs.size(); i++) {
        if (outputs[i] != -1) {
            ...
            rv.push_back(n->fire_times); // If tracking is turned off, this will
                                         // be empty.
        }
    }
    return rv;
...
vector<vector<double>> Network::neuron_vectors() {
    ...
    for (i = 0; i < sorted_neuron_vector.size(); i++) {
        n = sorted_neuron_vector[i];
        rv.push_back(n->fire_times); // JSP: If tracking is turned off, this
                                     // will be empty.
    }
    return rv;
}
...
vector<double> Network::neuron_pre_charges() {
    vector<double> rv;

    for (size_t i = 0; i < this->_neuron_pre_charges.size(); i++) {
        rv.push_back(this->_neuron_pre_charges[i]);
    }
    return rv;
}
...
Processor::Processor(json& params) {
    ...
    if (params.contains("input_scaling_value")) {
        ...
        estring +=
            "      Please change that parameter to spike_value_factor.\n";
        throw SRE(estring);
    }
    ...
    if (params.contains("specific_weights")) {
        ...
        estring += "      Instead, you simply set the weights array, and it "
                   "will be used.\n";
        estring += "      Please also see utils/risp_08_2024.\n";
        throw SRE(estring);
    }
    ...
    if (params.contains("noisy_weights")) {
        ...
        estring += "      Instead, you simply set the stds array, and it will "
                   "be used.\n";
        estring += "      Please also see utils/risp_08_2024.\n";
        throw SRE(estring);
    }
    ...
    if (params.contains("weights") && params["weights"].size() > 0) {
        ...
        if (params.contains("min_weight")) {
            throw SRE("RISP: Cannot have weights and min_weight.");
        }
        if (params.contains("max_weight")) {
            throw SRE("RISP: Cannot have weights and max_weight.");
        }
        if (!params.contains("inputs_from_weights")) {
            throw SRE("RISP: If you specify weights, then you must specify "
                      "inputs_from_weights.");
        }

        for (i = 1; i < weights.size(); i++) {
            if (weights[i] < weights[i - 1]) {
                throw SRE("RISP: If you specify weights, then they must be "
                          "sorted.\n");
            }
            if (discrete && weights[i] - ((int)weights[i]) != 0) {
                throw SRE("RISP: Since discrete is true, weights must be "
                          "integers.\n");
            }
        }
        ...
        if (!inputs_from_weights && !params.contains("spike_value_factor")) {
            throw SRE("RISP: If inputs_from_weights is false, you must specify "
                      "spike_value_factor.");
        }

        if (inputs_from_weights && params.contains("spike_value_factor")) {
            throw SRE("RISP: If inputs_from_weights is true, you cannot "
                      "specify spike_value_factor.");
        }
        ...
        if (!params.contains("min_weight")) {
            throw SRE("RISP: Need parameter min_weight.");
        }
        if (!params.contains("max_weight")) {
            throw SRE("RISP: Need parameter max_weight.");
        }
        if (params.contains("inputs_from_weights")) {
            throw SRE("RISP: If you don't specify weights, you cannot specify "
                      "inputs_from_weights.");
        }
    ...
    /* Spike_value_factor has to be set after discrete and threshold_inclusive
     * have been set. */

    if (params.contains("spike_value_factor")) {
        spike_value_factor = params["spike_value_factor"];

        /* If there are weights, and spike_value_factor is unspecified, then
           we're using inputs_from_weights, and spike_value_factor is
           meaningless. I'm going to set it to a big number to help debug. */
        ...
        if (max_weight < max_threshold ||
            (!threshold_inclusive && max_weight == max_threshold)) {
            fprintf(stderr,
                    "Warning: max_weight <%s max_threshold and "
    ...
    if (params.contains("stds")) {
        stds = params["stds"].get<vector<double>>();
    }
    ...
    if (discrete) {
        if (!is_integer(min_weight)) {
            throw SRE("When discrete = true, min_weight must be an integer.");
        }
        if (!is_integer(max_weight)) {
            throw SRE("When discrete = true, max_weight must be an integer.");
        }
        if (!is_integer(min_potential)) {
            throw SRE(
                "When discrete = true, min_potential must be an integer.");
        }
        if (!is_integer(min_threshold)) {
            throw SRE(
                "When discrete = true, min_threshold must be an integer.");
        }
        if (!is_integer(max_threshold)) {
            throw SRE(
                "When discrete = true, max_threshold must be an integer.");
        }
        for (i = 0; i < weights.size(); i++) {
            if (!is_integer(weights[i])) {
                throw SRE(
                    "When discrete = true, all weights must be integers.");
            }
        }
    }
    if (min_potential > 0) {
    ...
    if (stds.size() != 0 && weights.size() == 0) {
        throw SRE("If you specify stds, then you must specify weights and they "
    ...
    if (stds.size() != 0 && discrete) {
        throw SRE("Cannot specify stds and discrete = true.");
    }
    if (noisy_stddev != 0 && discrete) {
        throw SRE("Cannot specify noisy_stddev and discrete = true.");
    }
    ...
    if (noisy_seed != 0) {
        saved_params["noisy_seed"] = noisy_seed;
    }
    if (noisy_stddev != 0) {
        saved_params["noisy_stddev"] = noisy_stddev;
    }
    if (stds.size() != 0) {
        saved_params["stds"] = stds;
    }
};
...
bool Processor::load_network(neuro::Network* net, int network_id) {
    ...
    if (!net->is_edge_property("Delay")) {
        error += (rln + "Missing edge Delay property\n");
    }
...
vector<double> Processor::neuron_pre_charges(int network_id) {
    return get_risp_network(network_id)->neuron_pre_charges();
}
...
void Processor::synapse_weights(vector<uint32_t>& pres, vector<uint32_t>& posts,
...
PropertyPack Processor::get_network_properties() const {
    ...
    pp.add_node_property("Threshold", min_threshold, max_threshold,
                         (discrete) ? Property::Type::INTEGER
    ...
    if (weights.size() > 0) {
        pp.add_edge_property("Weight", 0, weights.size() - 1,
...
string Processor::get_name() const { return "risp"; }
...
Network* Processor::get_risp_network(int network_id) {
    ...
    if (it == networks.end()) {
        snprintf(
            buf, 200,
            "risp::Processor::get_risp_network() network_id %d does not exist",
            network_id);
        throw SRE((string)buf);
    }
    return it->second;
