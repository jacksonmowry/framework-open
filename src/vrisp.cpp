...
Network::Network(neuro::Network* net, double _min_potential, char leak,
    ...
    neuron_threshold.resize(allocation_size, max_threshold);
...
void Network::process_events(uint32_t time) {
    ...
    for (size_t i = 0; i < neuron_count; i += 8) {
        ...
        fired |= (fired & 0x6060606060606060) << 1;
        int8_t fired_arr[8];
        *(uint64_t*)fired_arr = fired;

        // for (size_t j = 0; j < 8; j++) {
        //     printf("charge: %hhd, threshold: %hhd\n",
        //            neuron_charge_buffer[internal_timestep * allocation_size +
        //                                 i + j],
        //            neuron_threshold[i + j]);
        // }
        // puts("");

        for (size_t neuron = 0; neuron < 8; neuron++) {
            if (fired_arr[neuron] >= 0) {
                // Neuron fired
