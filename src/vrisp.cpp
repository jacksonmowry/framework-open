...
#if defined(NO_SIMD) || defined(RISCVV_SYNAPSES) || defined(SWAR)
static bool
...
void Network::apply_spike(const Spike &s, bool normalized) {
  ...
  if (normalized && (s.value < -1 || s.value > 1)) {
    throw SRE("vrisp::Network::apply_spike() - value (" + to_string(s.value) +
              ") must be in [-1,1].");
  }
...
void Network::process_events(uint32_t time) {
  ...
  for (size_t i = 0; i < neuron_count; i++) {
    ...
    if (neuron_charge_buffer[internal_timestep * allocation_size + i] >=
      ...
      if (!get_leak(neuron_leak, i)) {
        ...
        // timestep
        neuron_charge_buffer[(internal_timestep + 1) % tracked_timesteps_count *
                                 allocation_size +
                             i] +=
            neuron_charge_buffer[internal_timestep * allocation_size + i];
      }
  ...
  for (size_t i = 0; i < neuron_count; i += 8) {
    uint8_t charges[8] = {
        static_cast<uint8_t>(
            neuron_charge_buffer[internal_timestep * allocation_size + i + 0] >=
                    min_potential
                ? neuron_charge_buffer[internal_timestep * allocation_size + i +
                                       0]
                : min_potential),
        static_cast<uint8_t>(
            neuron_charge_buffer[internal_timestep * allocation_size + i + 1] >=
                    min_potential
                ? neuron_charge_buffer[internal_timestep * allocation_size + i +
                                       1]
                : min_potential),
        static_cast<uint8_t>(
            neuron_charge_buffer[internal_timestep * allocation_size + i + 2] >=
                    min_potential
                ? neuron_charge_buffer[internal_timestep * allocation_size + i +
                                       2]
                : min_potential),
        static_cast<uint8_t>(
            neuron_charge_buffer[internal_timestep * allocation_size + i + 3] >=
                    min_potential
                ? neuron_charge_buffer[internal_timestep * allocation_size + i +
                                       3]
                : min_potential),
        static_cast<uint8_t>(
            neuron_charge_buffer[internal_timestep * allocation_size + i + 4] >=
                    min_potential
                ? neuron_charge_buffer[internal_timestep * allocation_size + i +
                                       4]
                : min_potential),
        static_cast<uint8_t>(
            neuron_charge_buffer[internal_timestep * allocation_size + i + 5] >=
                    min_potential
                ? neuron_charge_buffer[internal_timestep * allocation_size + i +
                                       5]
                : min_potential),
        static_cast<uint8_t>(
            neuron_charge_buffer[internal_timestep * allocation_size + i + 6] >=
                    min_potential
                ? neuron_charge_buffer[internal_timestep * allocation_size + i +
                                       6]
                : min_potential),
        static_cast<uint8_t>(
            neuron_charge_buffer[internal_timestep * allocation_size + i + 7] >=
                    min_potential
                ? neuron_charge_buffer[internal_timestep * allocation_size + i +
                                       7]
                : min_potential)};
    uint64_t vec_a = *(uint64_t *)charges;
    vec_a |= 0x8080808080808080;
    uint64_t vec_b = *(uint64_t *)(neuron_threshold.data() + i);

    uint64_t fired = (vec_a - vec_b) & 0x7f7f7f7f7f7f7f7f;
    uint8_t fired_arr[8];
    *(uint64_t *)fired_arr = fired;

    for (size_t j = 0; j < 8; j++) {
      int8_t tmp = fired_arr[j];
      printf("%hhd - %hhd = %s\n", charges[j], neuron_threshold[j],
             !(fired >> (6 + (56 - (j * 8)))) ? "fired" : "not fired");
    }
    puts("");

    for (size_t neuron = 0; neuron < 8; neuron++) {
      if (!(fired >> (6 + (56 - (neuron * 8))))) {
        // Neuron fired
        for (size_t j = 0; j < synapse_to[i + neuron].size(); j++) {
          neuron_charge_buffer[((internal_timestep +
                                 synapse_delay[i + neuron][j]) %
                                tracked_timesteps_count) *
                                   allocation_size +
                               synapse_to[i + neuron][j]] +=
              synapse_weight[i + neuron][j];
        }

        // Track output count and last fire time
        if (outputs[i + neuron]) {
          output_last_fire_timestep[i + neuron] = time;
          output_fire_count[i + neuron]++;
        }
      } else {
        // Neuron didn't fire
        if (!get_leak(neuron_leak, i + neuron)) {
          // If we don't leak we carry this charge over into the next
          // timestep
          neuron_charge_buffer[(internal_timestep + 1) %
                                   tracked_timesteps_count * allocation_size +
                               i + neuron] +=
              neuron_charge_buffer[internal_timestep * allocation_size + i +
                                   neuron];
        }
  ...
  for (size_t i = 0; i < neuron_count; i += max_vector_length) {
    ...
    charges = __riscv_vmax_vv_i8m1(charges, min_potential_vec, vector_length);
    ...
    if (leak_mode != 'a') {
      vbool8_t leak = __riscv_vlm_v_b8(&neuron_leak[i / 8], vector_length);
      ...
      next_charges = __riscv_vadd_vv_i8m1(next_charges, charges, vector_length);

      __riscv_vse8_v_i8m1_m(should_carryover,
                            &neuron_charge_buffer[((internal_timestep + 1) %
                                                   tracked_timesteps_count) *
                                                      allocation_size +
                                                  i],
    ...
    for (size_t j = 0; j < vector_length; j++) {
      ...
      for (size_t k = 0; k < num_outgoing; k += max_vector_length) {
        size_t vector_length = min((size_t)max_vector_length, num_outgoing - k);

        vint8m1_t weights =
            __riscv_vle8_v_i8m1(&synapse_weight[i + j][k], vector_length);
        vuint8m1_t delays =
            __riscv_vle8_v_u8m1(&synapse_delay[i + j][k], vector_length);
        ...
        indexes = __riscv_vremu_vx_u16m2(indexes, tracked_timesteps_count,
                                         vector_length);

        // vmadd.vx vd, rs1, vs2, vm | vd[i] = (x[rs1] * vd[i]) + vs2[i]
        indexes = __riscv_vmadd_vx_u16m2(indexes, (uint16_t)allocation_size,
        ...
        downstream_charges =
            __riscv_vadd_vv_i8m1(downstream_charges, weights, vector_length);
  ...
  for (size_t i = 0; i < neuron_count; i += max_vector_length) {
    ...
    for (size_t j = 0; j < vector_length; j++) {
      ...
      for (size_t k = 0; k < synapse_to[i + j].size(); k++) {
        neuron_charge_buffer[((internal_timestep + synapse_delay[i + j][k]) %
                              tracked_timesteps_count) *
                                 allocation_size +
                             synapse_to[i + j][k]] += synapse_weight[i + j][k];
      }
    }
  }
#endif
...
void Network::synapse_weights(vector<uint32_t> &pres, vector<uint32_t> &posts,
  ...
  for (size_t i = 0; i < neuron_count; i++) {
    for (size_t j = 0; j < synapse_to[i].size() && j < synapse_weight[i].size();
         j++) {
      pres.push_back(i);
...
void Network::clear_activity() {
  ...
  fill(output_last_fire_timestep.begin(), output_last_fire_timestep.end(), -1);
